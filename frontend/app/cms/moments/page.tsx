...
export default function CmsMomentsPage() {
  const router = useRouter();
  const [moments, setMoments] = useState<CmsMomentItem[]>([]);
  ...
  async function fetchMoments() {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/cms/moments`,
        {
  ...
  async function handleDelete(momentId: number) {
    if (!confirm("Are you sure you want to delete this moment? All photos will be permanently removed.")) return;
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/cms/moments/${momentId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Moments</h1>
        <Link
          href="/cms/moments/new"
              ...
              {moments.map((moment) => (
                <tr key={moment.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm space-x-2">
                    <Link
                      href={`/cms/moments/${moment.id}/edit`}
