...
async function getMoments(): Promise<PaginatedMoments> {
  const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
  const res = await fetch(`${apiUrl}/api/moments`, { next: { revalidate: 60 } });
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      {data.items.map((moment) => (
        <a
          ...
          className="block rounded-lg border border-pink-200 bg-pink-50/40 overflow-hidden hover:border-pink-300 hover:shadow-sm transition-all"
        >
          <div className="aspect-[4/3] relative bg-gray-100">
            {moment.cover_photo_url ? (
              <img
                src={moment.cover_photo_url}
                alt={moment.title}
